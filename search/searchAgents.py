...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        space)
        """
        return(self.startingPosition,[]) #starting position will get us just the start state of the game through getpacmanstate
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            dx, dy = Actions.directionToVector(action)
            nextx, nexty = int(x + dx), int(y + dy)
            block = self.walls[nextx][nexty]
            if not block:
                nextcorner = list(exploredcorner)
                nextpos = (nextx, nexty)
                if nextpos in self.corners:
                    if not nextpos in nextcorner:
                        nextcorner.append(nextpos)
                successor = ((nextpos, nextcorner), action, 1)
