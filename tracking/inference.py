...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for x in range(X_RANGE):
        for y in range(Y_RANGE):
            inrange_position.append((x,y))#domain of all the possible positions ghost or pacman can take
            #print(inrange_position)
    for pos1 in inrange_position:
        for pos2 in inrange_position:
            if (manhattanDistance(pos1,pos2)+MAX_NOISE) not in inrange_length:
                inrange_length.append(manhattanDistance(pos1,pos2)+MAX_NOISE)#domain of the dist btw all possible positions of pacman and respective ghosts accounting for the noise
            if abs(manhattanDistance(pos1,pos2)-MAX_NOISE) not in inrange_length:
                inrange_length.append(abs(manhattanDistance(pos1,pos2)-MAX_NOISE))
    variableDomainsDict[PAC]=inrange_position
    variableDomainsDict[GHOST0]=inrange_position
    variableDomainsDict[GHOST1]=inrange_position
    variableDomainsDict[OBS0]=inrange_length
    variableDomainsDict[OBS1]=inrange_length
    "*** YOUR CODE HERE ***"
    #raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)#getting all factors
        for joinVariable in eliminationOrder:
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, joinVariable)#joining the variable tobe removed
            if len(joinedFactor.unconditionedVariables()) > 1:
                eliminatedFactor = eliminate(joinedFactor, joinVariable)#now eliminating the joined variable
